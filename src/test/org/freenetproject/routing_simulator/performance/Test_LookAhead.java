...
public class Test_LookAhead {
	...
	public void benchMarkLookAhead() throws Exception {
		...
		/*
		 * The original run times for this test were:
		 * 174042ms
		 * 178212ms
		 * 176805ms
		 * 
        ...
        String[] args = new String[] { "--link-ideal", "--degree-fixed", "10",
                "--graph-size", "1000", "--route", "100000",
                "--route-hops", "10000",  "--route-fold-policy", "NONE",
                "--route-look-ahead", "3"
                 };
        long startTime = new Date().getTime();
        new RoutingSim().run(Arguments.parse(args));
        System.out.println("run time: " + (new Date().getTime() - startTime) + "ms");
