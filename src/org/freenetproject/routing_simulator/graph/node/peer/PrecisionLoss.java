...
public class PrecisionLoss extends LoopDetection {
    ...
    public PrecisionLoss(final FoldingPolicy policy, long requestId, double precisionLoss) {
        super(policy, requestId);
    ...
    protected double calculateDifference(SimpleNode n, int lookAhead, double target) {
        ...
        double remainingPrecision = Math.pow( 1 - this.precisionLoss , lookAhead - 1);
        
        String locationText = new DecimalFormat("#.#######################").format( n.getLocation()); 
        ...
        String reducedLocation = locationText.substring(0, integerPlaces + (int) Math.ceil(reducedPlaces) + 1);
