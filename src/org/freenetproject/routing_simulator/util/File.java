...
public class File {
	...
	public static java.io.File writableDirectory(String path) {
		...
		if (!file.exists()) {
			if (!file.mkdirs()) {
				System.out.println("Unable to create the output directory \"" + file.getAbsolutePath() + "\".");
				return null;
			} else {
				System.out.println("The output directory \"" + file.getAbsolutePath() + "\" did not exist, so it was created.");
			}
		} else if (!file.isDirectory()) {
			System.out.println("The output path \"" + file.getAbsolutePath() + "\" is not a directory as expected.");
			return null;
		} else if (!file.canWrite()) {
			System.out.println("No write access to the output directory \"" + file.getAbsolutePath() + "\".");
			return null;
		}
		return file;
	}
