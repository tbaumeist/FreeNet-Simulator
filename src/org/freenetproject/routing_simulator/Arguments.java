...
public final class Arguments {
     ...
     * Program options
     */
    private static final Option OPT_HELP = new Option("h", "help", false, "Display this message.");
    private static final Option OPT_VERSION = new Option("v", "version", false, "Display software version.");
    private static final Option OPT_LOG_LEVEL = new Option("l", "log-level", true, "Level of logging: ");
    private static final Option OPT_SEED = new Option("s", "seed", true, "Seed used by psuedorandom number generator.");
    private static final Option OPT_PAUSE = new Option("p", "pause", false, "Pause the program before execution. (Allows external programs to be attached).");

    /*
     * Graph generation options
     */
    private static final Option OPT_GRAPH_LOAD = new Option("gl", "graph-load", true, "Path to load a saved graph from.");
    private static final Option OPT_GRAPH_SAVE_DOT = new Option("gsd", "graph-save-dot", true, "Path to save a graph after simulation is run on it in plain text (DOT format).");
    private static final Option OPT_GRAPH_SAVE = new Option("gs", "graph-save", true, "Path to save a graph after simulation is run on it.");
    private static final Option OPT_GRAPH_SANDBERG = new Option("gS", "graph-sandberg", true, "Generate a directed graph with an edge from x to x -1 mod N for all x = 0 ... N - 1 as described in early section 2.2.1 in \"Searching in a Small World.\" Takes the number of shortcuts to make; the paper specifies 1 shortcut.");
    private static final Option OPT_GRAPH_SUPER_NODE = new Option("gU", "graph-supernode", false, "Generate a graph with all nodes connected to a single supernode.");
    private static final Option OPT_GRAPH_SIZE = new Option("gn", "graph-size", true, "Number of nodes in the network.");
    private static final Option OPT_GRAPH_LATTICE = new Option("gL", "graph-lattice", false, "Generate a graph with undirected lattice links, the given degree distribution, and the given link length distribution. e.g. Topology starts out as a circle.");
    private static final Option OPT_GRAPH_FAST_LOCATION = new Option("gfl", "graph-fast-location", false, "If present, the simulator will assign locations with even spacing and, when using --ideal-link, take shortcuts to speed up graph generation.");

    private static final Option OPT_DEGREE_OUTPUT = new Option("do", "degree-output", true, "Output file for degree distribution.");
    private static final Option OPT_DEGREE_FIXED = new Option("df", "degree-fixed", true, "All nodes are as close to the specified degree as practical.");
    private static final Option OPT_DEGREE_CONFORMING = new Option("dc", "degree-conforming", true, "Distribution conforming to a file. Takes a path to a degree distribution file of the format \"[degree] [number of occurrences]\\n\"");
    private static final Option OPT_DEGREE_POISSON = new Option("dp", "degree-poisson", true, "Distribution conforming to a Poisson distribution with the given mean.");

    private static final Option OPT_LINK_OUTPUT = new Option("lo", "link-output", true, "Output file for link length distribution.");
    private static final Option OPT_LINK_EXCLUDE_LATTICE = new Option("lel", "link-exclude-lattice", false, "Exclude links from index X to X - 1 mod N when outputting link lengths. If the graph does not actually have lattice connections this is not recommended. If not specified any lattice links will be included when outputing link length.");
    private static final Option OPT_LINK_IDEAL = new Option("li", "link-ideal", false, "Kleinberg's ideal distribution: proportional to 1/d.");
    private static final Option OPT_LINK_FLAT = new Option("lf", "link-flat", false, "Intentionally terrible distribution: uniformly random.");
    private static final Option OPT_LINK_CONFORMING = new Option("lc", "link-conforming", true, "Distribution conforming to a file. Takes a path to a degree distribution file of the format \"[degree] [number of occurrences]\\n\"\"");

    private static final Option OPT_ROUTE = new Option("r", "route", true, "Simulate routing the given number of requests. Requires that --route-output and --route-hops be specified.");
    private static final Option OPT_ROUTE_HOPS = new Option("rh", "route-hops", true, "The maximum number of hops to route with.");
    private static final Option OPT_ROUTE_OUTPUT = new Option("ro", "route-output", true, "The file to which routing information is output.");
    private static final Option OPT_ROUTE_FOLDING_POLICY = new Option("rfp", "route-fold-policy", true,  "Path folding policy:");
    private static final Option OPT_ROUTE_LOOK_AHEAD = new Option("rla", "route-look-ahead", true, "When routing look ahead n hops before routing from a node. Default = 1.");
    private static final Option OPT_ROUTE_POLICY = new Option("rp", "route-policy", true, "Routing policy used.");
    private static final Option OPT_ROUTE_BOOTSTRAP = new Option("rb", "route-bootstrap", false, "If specified, nodes which lose all their connections due to path folding will be connected to random nodes.");
    private static final Option OPT_ROUTE_OLD_FOLDING = new Option("rop", "route-old-path-fold", false, "Use the old path folding mechanism. (7% chance to randomly path fold).");

    private static final Option OPT_PROBE = new Option("p", "probe", true, "Simulate running probes from random locations for the specified number of maximum hops. Requires that --probe-output be specified.");
    private static final Option OPT_PROBE_METROPOLIS_HASTINGS = new Option("pmh", "probe-metropolis-hastings", false, "If present, probes will be routed with Metropolis-Hastings correction. If not, peers will be selected entirely at random.");
    private static final Option OPT_PROBE_OUTPUT = new Option("po", "probe-output", true, "Directory to which probe distribution is output as \"[node ID] [times seen]\\n\" for a reference of random selection from the whole and at each hop up to the specified maximum hops.");
    ...
    private Arguments(final boolean lattice, final boolean fastGeneration,
            ...
            final boolean oldPathFolding, final CommandLine cmd) {
    ...
    public static Arguments parse(final String[] args) throws Exception {
        ...
        int linkOptions = 0;
        for (String option : new String[] {
                OPT_LINK_IDEAL.getLongOpt(),
                OPT_LINK_FLAT.getLongOpt(),
                OPT_LINK_CONFORMING.getLongOpt() }) {
            if (cmd.hasOption(option)) {
                linkOptions++;
            }
        }
        ...
        if (degreeOptions > 1 || linkOptions > 1) {
            System.out
                    .println("Graph cannot be generated with multiple methods at once.");
            return null;
        }
        ...
        if (!cmd.hasOption(OPT_GRAPH_SIZE.getLongOpt())
                && graphGenerator != GraphGenerator.LOAD) {
            System.out.println("Network size not specified. (--"
        ...
        if (validGeneration == 0 || validGeneration > 1) {
            System.out
                    .println("Either zero or too many graph generation methods specified.");
            System.out.println("Valid graph generators are:");
            System.out.println(" * --" + OPT_GRAPH_LOAD.getLongOpt());
            System.out.println(" * --" + OPT_GRAPH_SANDBERG.getLongOpt()
                    + " with --link-* and --" + OPT_GRAPH_SIZE.getLongOpt());
            System.out.println(" * --degree-*, --link-*, --"
                    + OPT_GRAPH_SIZE.getLongOpt() + ", and optionally --"
                    + OPT_GRAPH_LATTICE.getLongOpt());
            System.out.println(" * --" + OPT_GRAPH_SUPER_NODE.getLongOpt()
                    + " with --" + OPT_GRAPH_SIZE.getLongOpt()
                    + " and optionally --" + OPT_GRAPH_LATTICE.getLongOpt());
            return null;
        }
        ...
        if (cmd.hasOption(OPT_ROUTE.getLongOpt())
            ...
            System.out.println("--" + OPT_ROUTE.getLongOpt()
                    + " was specified, but not --"
                    + OPT_ROUTE_HOPS.getLongOpt() + ".");
            return null;
        }
        if (cmd.hasOption(OPT_ROUTE.getLongOpt())
                && !cmd.hasOption(OPT_ROUTE_OUTPUT.getLongOpt())) {
            System.out.println("--" + OPT_ROUTE.getLongOpt()
                    + " was specified, but not --"
                    + OPT_ROUTE_OUTPUT.getLongOpt() + ".");
            return null;
        }

        if (cmd.hasOption(OPT_PROBE.getLongOpt())
                && !cmd.hasOption(OPT_PROBE_OUTPUT.getLongOpt())) {
            System.out.println("--" + OPT_PROBE.getLongOpt()
                    + " was specified, but not --"
        ...
        return new Arguments(lattice, fastGeneration, cmd.hasOption(OPT_PROBE
                ...
                logLevel, pause, oldPathFolding, cmd);
    }
}
